// Forked plugin for `react-native-worklets@0.7.2`.
//   Changed imports:
//     `require("@babel/core")` → `require("snack-babel-standalone")`
//     `require("@babel/generator")` → `require("snack-babel-standalone").generator`
//     `require("@babel/types")` → `require("snack-babel-standalone").types`
//     `require("@babel/traverse") → `require("snack-babel-standalone").traverse`
//   Other changes:
//     `lib/workletStringCode.js` → `includeSourceMap` must be set to `false` in to avoid using `fs` in React Native
//     `lib/workletStringCode.js` → `var fs = __importStar(require("fs"))` must be set to an empty object to avoid using `fs` in React Native
//     `lib/generate.js` → Method is disabled to avoid using `fs`, `path`, and `assert` in React Native
...
var require_workletStringCode = __commonJS({
  "lib/workletStringCode.js"(exports2) {
    ...
    var fs = {}; // Disabled due to no `fs` available in React Native
...
var require_workletFactory = __commonJS({
  "lib/workletFactory.js"(exports2) {
    ...
    var REAL_VERSION = "0.7.2"; // Hardcoded to avoid importing non-existing files
